...
| Notebook kernels outlived their notebook: `start_kernel_tasks` held the session strongly from a detached task, so closing the tab could not drop the editor and the kernel's `Drop` never killed the process, and nothing killed it on quit either. The upstream branch's tests differ from the carried copy (they predate the #64274 test helper), so take upstream's version of the test module if it lands. **The PR was closed**: upstream wants kernel lifetime handled in one place rather than a second quit path bolted onto the notebook, so this stays fork-local until that redesign is agreed with them | `crates/repl/src/kernels/mod.rs`, `native_kernel.rs`, `remote_kernels.rs`, `ssh_kernel.rs`, `wsl_kernel.rs`, `crates/repl/src/notebook/notebook_ui.rs` | [zed#64275](https://github.com/zed-industries/zed/pull/64275) (closed) |

...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    ...
    // Suzuri installs its own command name so it never clobbers a real Zed
    // install's `zed` symlink.
    let link_path = Path::new("/usr/local/bin/suzuri");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        let path = match install_script(cx.deref()).await {
            ...
            Ok(None) => return Ok(()),
            Err(error) => {
                log::error!("failed to install suzuri CLI: {error:#}");
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `suzuri` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the Suzuri CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            ...
            workspace.show_toast(
                Toast::new(
                    ...
                    format!(
                        "Installed `suzuri` to {}. You can launch Suzuri from your terminal.",
    ...
    .detach_and_prompt_err("Cannot install the Suzuri CLI", window, cx, |_, _, _| None);
}

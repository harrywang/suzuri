...
//!
//! Citations are the reason the work is shaped this way. A vault that keeps a
//! BibLaTeX library and cites with Pandoc's own `[@key]` syntax gets a
//! formatted bibliography from Pandoc's citeproc, in whatever CSL style the
//! note's frontmatter names. Nothing about that had to be invented here; the
//! three pieces only had to be handed over.
...
mod tests {
    ...
    fn settings_are_registered_and_default_to_typst(cx: &mut gpui::App) {
        ...
        // `from_settings` falls back to the same constants when the section
        // is missing, so reading the store alone cannot tell a registered
        // section from an absent one. The raw text can: the section must be
        // a top-level key, at this file's two-space indent, or a merge that
        // slid it inside a neighbor would leave every value silently unread.
        assert!(
            defaults.contains("\n  \"markdown_export\": {"),
            "default.json has no top-level markdown_export section"
        );
